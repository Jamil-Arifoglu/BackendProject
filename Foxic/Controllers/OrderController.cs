    ...
    public class OrderController : Controller
    {
        ...
        public async Task<IActionResult> AddBasket(int clotherId, Clothes basketClother)
        {
            if (User.Identity.IsAuthenticated)
            {
                ...
                BasketItem item = new()
                {
                    ClothesColorSizeId = clother.ClothesId,
                    SaleQuantity = basketClother.AddCart.Quantity,
                    UnitPrice = clother.Clothes.Price
                };
        ...
        public IActionResult GetSizes(int clothesId, int colorId)
        {
            List<ClothesColorSize> clothesSizeColor = _context.ClothesColorSizes
                                                       .Include(p => p.Size).Where(p => p.ClothesId == clothesId && p.ColorId == colorId).ToList();
            if (clothesSizeColor is null) return Json(new { status = 404 });
            var sizes = clothesSizeColor.Select(p => new { Id = p.SizeId, p.Size.Name }).ToList();
            return Json(sizes);
        }
    }
}
